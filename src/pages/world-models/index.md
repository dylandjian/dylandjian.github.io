...
The encode and decode methods do as their name suggest. The reparameterize method is used to sample from the mean and standard deviation during training, and returns the mean when infering. However, to make sure that our M model doesn't overfit to a specific latent representation while learning how to model the time dependency on the space, the sampling method is also used even during inference time.
